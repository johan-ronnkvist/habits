...
def parse_url_date(date_path: str) -> date:
    ...
    path_parts = [
        part.lower().strip() for part in date_path.strip("/").split("/") if part.strip()
    ]
        ...
        if len(path_parts) == 1:
            ...
            if _is_year(path_parts[0]):
                ...
                raise DateParseError(
                    f"Cannot determine year in: {'/'.join(path_parts)}"
                )
...
def _validate_not_future(target_date: date) -> None:
    ...
    if target_date > today:
        raise DateParseError(
            f"Cannot navigate to future date: {target_date.isoformat()} (today is {today.isoformat()})"
        )
